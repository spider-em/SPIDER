...
#
# PURPOSE:  Converts ascii manuals of SPIDER in $oldman_dir to HTML 
#           (only files with .man  extensions are processed), and 
#           places the HTML files in $rawdocs_dir).
#           Also makes index of the operations.
#
...
$hr          = qq(<p>\n);
$man_url     = qq(man);                     # URL location for a man page wrt rawdocs
$rawdocs_dir = qq($spider_root/rawdocs);
$index_dir   = $rawdocs_dir;                # Directory for the html index 
$src_dir     = qq($spider_root/src);
$htmlext     = qq(html);                    # File extension for html files 
$oldman_dir  = qq($spider_root/man);        # Directory for ascii man pages 
$exa_dir     = qq($spider_root/docs/exa);   # Directory for example man pages 
$ferr        = "htdocs_warnings.LOG";       # Error File
...
sub lineprepare  # --------------- lineprepare ---------------------
   ...
   if ($line =~s/-\s*\n$//)
       ...
       print ferr "$oldman_dir/$fn.$ext: possible hyphenation: $_ \n";
       }
   $line =~s/\n$//;
   &htmlescape;
   }
...
sub headerscan  # ---------------- headerscan -----------------------
   ...
   unless ($oper=~/[^a-z]/)
      {print ferr "$oldman_dir.$ext/$fn: strange header\n";};
      
   $defn = join(' ',@words);
   unless ($defn =~/[a-z]/) 
      {print ferr "$oldman_dir/$fn.$ext: Definition: $defn -- is in caps only\n";}
   ...
   $htmlurl{$oper}    = "$man_url/$fn.$htmlext";	# Also an associative array!
   ...
   while(@lines)
      ...
      if ($line=~/\w/)
         ...
         if (-T "$oldman_dir/$fn.man")
            ...
            print ferr "$oldman_dir/$fn.$ext: Reference $oldman_dir/$fn.* not found!\n";
            }
         }
      }	
      print TOO qq(</table>);
...
sub getdescscan # ---------------- getdescscan -----------------------
   ...
   unless (open(FROE, "< $oldman_dir/$loperr.man"))               
      {
...
sub includescan # ---------------- includescan -----------------------
   ...
   unless (open(FROI, "< $oldman_dir/$incfile"))               
      {
      print      "From: $fn.$ext  Can not include: $oldman_dir/$incfile\n";
      print ferr "From: $fn.$ext  Can not include: $oldman_dir/$incfile\n";
      return;
      }
...
sub usagescan  # ---------------- usagescan -----------------------
   ...
   # if ($oper ne $line) {print ferr "$oldman_dir/$fn.$ext: Strange .OPERATION\n";}
...
sub subscan  # -------------------------------- subscan --------------------------------
   ...
   foreach $word (@words)
      ...
      if ($nosource)
         ...
         { 
         print ferr "$oldman_dir/$fn.$ext: $src_dir/$word.f not found\n";
         print TOO qq($wordd);
         }
      }
   print TOO qq(</dl>\n);		
...
opendir(oldman_dir,$oldman_dir) 
   || die "Can't open $oldman_dir\n";
...
foreach $fn (@files)
   ...
   if ($fn eq "pocketcalc") 
      {next f;}
   unless (-T "$oldman_dir/$fn.$ext") 
      {next f;}	# Not a valid file

   # Run cpp to add include files, option -P strips first line == cpp id #
   if ($fn =~/^cp/)
      {open(fro, "/usr/bin/cpp -P -traditional $oldman_dir/$fn.$ext |") || die "Can't open $oldman_dir/$fn.$ext\n";}
   else
      {open(fro, "< $oldman_dir/$fn.$ext") || die "Can't open $oldman_dir/$fn.$ext\n";}

   open(TOO, "> $rawdocs_dir/$man_url/$fn.$htmlext") || die "Can't write to $rawdocs_dir/$man_url\n";
  
   # Find input file's atime & mtime
   ($atime, $mtime) = (stat("$oldman_dir/$fn.$ext"))[8,9];
   ...
   while(@lines)
      ...
      if ($line=~/^Filename:/)
         ...
         $outline=join (' ',split(' ',$line));
         print TOO qq($outline\n);
         print ferr "$oldman_dir/$fn.$ext: strange beginning\n";
         } 
      }
   ...
   utime($atime, $mtime, "$rawdocs_dir/$man_url/$fn.$htmlext");

   #print " Created: $rawdocs_dir/$man_url/$fn.$htmlext \n";
   }
print " \n";
print " Processed: $fnum  HTML SPIDER manual files in: $rawdocs_dir/$man_url \n";
print " ------------------------\n";
...
print " Created: $index_dir/operations_doc.html \n";
print " ------------------------\n";
