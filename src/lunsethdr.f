...
C           NORMAL STACKS HAVE ADDITIONAL OVERALL HEADER AT BEGINNING
      ...
      write(3,*)' '
      write(3,*)' In lunsetimgoff ================================='
      write(3,*)' In lunsetimgoff: imgnum,nrecs,labrec: ',
     &                             imgnum,nrecs,labrec 
      write(3,*)' In lunsetimgoff: lunaraoff,lunstkoff: ',
     &                             lunaraoff,lunstkoff 
      write(3,*)' '
#endif
...
#include "LUNHDR.INC"
      ...
      CALL LUNGETIS_MRC(LUN,IS_MRC,IRTFLG)
      IF (IS_MRC) THEN
         CALL LUNGET_PIXSIZ_MRC(LUN,PIXSIZ,IRTFLG)
         RETURN
      ENDIF
...
C     DETERMINE IF MRC OR SPIDER HEADER OBJECT
      CALL LUNGETIS_MRC(LUN,IS_MRC,IRTFLG)
      IF (IS_MRC) THEN
         CALL LUNSET_PIXSIZ_MRC(LUN,PIXSIZ,IRTFLG)
         RETURN
      ENDIF
...
C     SET NEW FILENAME IN HEADER OBJECT                
      IF (IS_MRC) THEN
         CALL LUNSET_FILE_MRC(LUN,FILNAM(1:NLET),'N',IRTFLG)
      ELSE
...
C     POINT TO HEADER OBJECT
      IF (IS_MRC) THEN
         CALL LUNGET_ISBARE_MRC(LUN,ISBARE,IRTFLG)
